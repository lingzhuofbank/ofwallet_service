...
//int to byte
func IB (tx *Transaction ) []byte{
	origin := []byte("0x")
	country_code := []byte {0, 0, 0, 0, 156}		// default ISO 'CHN'
	if tx.To().String() == "" && bytes.Compare(tx.Data(),origin ) != 0  {
		country_code[1]= tx.data.ACcode[1]
		country_code[2]= tx.data.ACcode[2]
		country_code[3]= tx.data.ACcode[3]
		country_code[4]= tx.data.ACcode[4]
		return 	country_code
	}

	return tx.data.ACcode
}
...
// ERROR -- FIXING
	...
	ofc_addr = append(tx.ACcode(), eth_addr...)
