...
var fk = [4]uint32{
	...
	0x5356a350, 0xa13afac7, 0x7d9971d9, 0x2bcd07dc,
}
...
var ck = [32]uint32{
	...
	0x70777e85, 0x00070e15, 0x1c232a31, 0x383f464d,
	0x545b6269, 0x8c939aa1, 0xa8afb6bd, 0xa0a7aeb5,
	0xc4cbd2d9, 0xe0e7eef5, 0x646b7279, 0x30373e45,
	0xdce3eaf1, 0xfc030a11, 0x181f262d, 0x343b4249,
	0x6c737a81, 0x888f969d, 0xa4abb2b9, 0x50575e65,
	0xc0c7ced5, 0xf8ff060d, 0x141b2229, 0x4c535a61,
	0x686f767d, 0x848b9299, 0x484f565d, 0xbcc3cad1,
...
func (c *Sm4Cipher) Encrypt(dst, src []byte) {
	...
	iv := [16]byte{0x62, 0xf2, 0xa1, 0xdf, 0x25, 0x34, 0x06, 0x0f, 0xa6, 0x67, 0x12, 0xb6, 0x55, 0x3f, 0x5e, 0x15}
	datalen := len(src)

	if datalen%16 != 0 {
		datalen = datalen/16*16 + 16
	}

	input := make([]byte, datalen)
	copy(input, src)
	count := len(input)/16 - 1
	for i := 0; i < 16; i++ {
		input[i] ^= iv[i]
	}
	cryptBlock(c.subkeys, c.block1, c.block2, dst, input, false)

	tmp := 1
	for count > 0 {
		count--
		for i := 0; i < 16; i++ {
			input[i+16*tmp] ^= dst[i+16*tmp-16]
		}
		//c.Encrypt(dst[16:], input[16:])
		cryptBlock(c.subkeys, c.block1, c.block2, dst[16*tmp:], input[16*tmp:], false)
		tmp++
	}
}
...
func (c *Sm4Cipher) Decrypt(dst, src []byte) {
	...
	for count > 0 {
		cryptBlock(c.subkeys, c.block1, c.block2, dst[16*count:], src[16*count:], true)
		for i := 0; i < 16; i++ {
			dst[i+16*count] ^= src[i+16*count-16]
		}
		count--
	}
